...
@Threads(1)
public class JdbcLookupBenchmarks {

  @Benchmark
  public int jdbcTemplateSqlIn(LookupState state) {
    List<JdbcBenchmarkRow> rows = state.environment.sqlIn(state.sql, state.codes);
    state.record(rows);
    return rows.size();
  }
