...
public class InsertBenchmarks {
  ...
  @Benchmark
  public int jpaSaveAllDefault(InsertState state) {
    return state.environment.defaultRepository().saveAll(state.rows).size();
  }

  @Benchmark
  public int jpaSaveAllBatched(InsertState state) {
    return state.environment.tunedRepository().saveAll(state.rows).size();
  }
  ...
  @Benchmark
  public int postgresBulkCopy(InsertState state) {
  ...
  public static class InsertState {
    ...
    public void verifyRows() {
      long actual = environment.rowCount();
      if (actual != size) {
        throw new IllegalStateException("expected " + size + " rows but found " + actual);
      }
    }
  }
}
